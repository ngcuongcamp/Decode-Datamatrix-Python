...
class MyApplication(QMainWindow):
    ...
    def handle_data_sfc(self, data):
        if self.THREAD_SFC_RC.is_running:
            print("\n\n")
            print("-----------DATA SFC ---------")
            print(f">>> Received signal from SFC: {data}")
            self.flag_wait_sfc = False

            logger.info(f">>> Received signal from SFC: {data}")
            if data == b"1":
    ...
    def handle_decode_data(self, data_decoded):
        if data_decoded == "2":
            ...
            # using push data to input instead of send to serial port
            self.keyboard.type(data_decoded)
    ...
    def thread_scan_action(self, progress, data_show, notification, data):
        try:
            if self.frame is not None:
                ...
                # test
                # frame = cv2.imread("./lab/65.png")
                # frame_crop = frame[70:350, 120:450]
                # resized_frame = self.reader._resize_image(frame_crop, 1.3)
                # images_processed = self.reader._process_image(
                #     resized_frame, option=1, ksize_value=3
                # )
    ...
    def decode_wrapper(
        ...
        if not stop_flag.is_set():
            ...
            # Nếu có kết quả, phát dữ liệu và dừng các luồng khác
            if data_decoded and not stop_flag.is_set():
                stop_flag.set()  # Dừng tất cả các luồng còn lại
                data.emit(data_decoded)  # Gửi kết quả
                notification.emit(("PASS", "PASS SCAN"))
                self.flag_wait_sfc = True
                stime = time.time()
                while self.flag_wait_sfc == True and time.time() - stime <= 2:
                    if self.flag_wait_sfc == False:
                        break
                if self.flag_wait_sfc == True:
                    notification.emit(("ERROR", "TIMEOUT SFC"))
                    data.emit("2")
            else:
                with counter_lock:
                    completed_threads[0] += 1  # Tăng biến đếm số luồng đã hoàn thành

                # Nếu cả ba luồng đã hoàn thành và không có kết quả, phát thông báo lỗi
                if completed_threads[0] == 3 and not stop_flag.is_set():
                    stop_flag.set()  # Đảm bảo không có luồng nào tiếp tục

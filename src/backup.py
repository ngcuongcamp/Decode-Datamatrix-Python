...
class MyApplication(QMainWindow):
    ...
    def __init__(self):
        ...
        # connect GUN PORT
        self.THREAD_GUN = Serial_Thread(self.COM_GUN, self.BAUDRATE_GUN)
        self.THREAD_GUN.data_received.connect(self.handle_data_gun)
        self.THREAD_GUN.start()
    ...
    def read_config(self):
        ...
        self.COM_GUN = config["SERIAL"]["COM_GUN"]
        self.BAUDRATE_GUN = int(config["SERIAL"]["BAUDRATE_GUN"])
    ...
    def thread_scan_action(self, progress, data_show, notification, data):
        try:
            if self.frame is not None:
                ...
                # data_decoded = self.reader._decode_pylibdmtx(self.frame)

                pylibdmtx_pool = threading.Thread(
                    target=self.decode_wrapper,
                    args=(self.reader._decode_pylibdmtx, stop_flag, data, notification),
                )

                zxingcpp_pool = threading.Thread(
                    target=self.decode_wrapper,
                    args=(self.reader._decode_zxingcpp, stop_flag, data, notification),
                )
                ...
                # else:
                #     data.emit(data_decoded)
                #     notification.emit(("PASS", "PASS SCAN"))
        except Exception as e:
            logger.error(f"Error in scan action: {e}")
            return None

    def decode_wrapper(self, decode_func, stop_flag, data, notification):
        if not stop_flag.is_set():
            data_decoded = decode_func(self.frame)
            if data_decoded and not stop_flag.is_set():
                stop_flag.set()  # Kích hoạt cờ để dừng luồng kia
                data.emit(data_decoded)
                notification.emit(("PASS", "PASS SCAN"))
            else:
                notification.emit(("ERROR", "ERROR SCAN"))
